...
<div class="tab-content">
    ...
    $this->db->order_by('id', 'DESC');
		$posts	=	$this->db->get_where('posts', array('author_id' => $this->session->userdata('author_id')))->result_array();
		...
		?>
            	...
            	<td style="text-align: left;"><?php echo $this->db->get_where('category', array('id' => $row['category']))->row()->english;?></td>
            	<td style="text-align: left;"><?php echo $this->db->get_where('author', array('author_id' => $row['author_id']))->row()->name;?></td>
				<td style="text-align: left;"><?php echo $row['posted_date'];?></td>
				<td style="text-align: left;"><?php if ($row['status'] == 1):?>
                ...
                <?php endif;?>
                <?php if ($row['status'] == 2):?>
                <div class="badge badge-danger" style="background-color: red;">
                  <?php echo get_phrase('Rejected');?>
                </div>
           		<?php endif;?></td>
			    <td style="text-align: center;" class="text-nowrap"><a href="<?php echo base_url();?>editor/post/edit/<?php echo $row['id'];?>" data-toggle="tooltip" data-original-title="Edit"> <i class="fa fa-edit text-danger m-r-10"></i> </a> </td>

           <td style="text-align: center;" class="text-nowrap"><a href="#" data-toggle="tooltip" onclick="confirm_modal('<?php echo base_url();?>editor/post/delete/<?php echo $row['id'];?>' , '<?php echo base_url();?>admin/reload_admin_list');" data-original-title="Delete"> <i class="fas fa-trash text-danger m-r-10"></i> </a></td>
                </tr>
                <?php endforeach;?>
              </tbody>
            </table>
	...
	<div class="tab-pane box" id="add" style="padding: 5px">
                <div class="box-content">
                	<?php echo form_open(base_url() . 'editor/members/create/' , array('class' => 'form-horizontal form-groups-bordered validate ajax-submit', 'enctype' => 'multipart/form-data'));?>
